                    ...
                    <tr>
                        <td style="padding: 35px 35px 15px;">
                            ...
                            <p style="color: #555; margin: 0; font-size: 15px; line-height: 1.7;">
                                A new order has been successfully placed on the EAY Sports storefront. Below are the order and shipping details.
                            </p>
                        </td>
                    </tr>
                            ...
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border: 1px solid #e9ecef; border-radius: 8px; padding: 20px;">
                                <tr>
                                    <td style="width: 50%; vertical-align: top;">
                                        <p style="color: #0f3460; font-weight: 700; font-size: 13px; margin: 0 0 8px; text-transform: uppercase;">Customer Info</p>
                                        <p style="margin: 0; font-size: 14px; color: #333;"><strong>Name:</strong> {{ $order->billing_name }}</p>
                                        <p style="margin: 4px 0; font-size: 14px; color: #333;"><strong>Email:</strong> {{ $order->billing_email }}</p>
                                        <p style="margin: 0; font-size: 14px; color: #333;"><strong>Phone:</strong> {{ $order->phone }}</p>
                                    </td>
                                    <td style="width: 50%; vertical-align: top;">
                                        <p style="color: #0f3460; font-weight: 700; font-size: 13px; margin: 0 0 8px; text-transform: uppercase;">Shipping & Payment</p>
                                        <p style="margin: 0; font-size: 14px; color: #333;"><strong>Address:</strong> {{ $order->shipping_address }}, {{ $order->city }}, {{ $order->zip_code }}</p>
                                        <p style="margin: 4px 0; font-size: 14px; color: #333;"><strong>Payment Method:</strong> {{ $order->payment_method }}</p>
                                        @if($order->notes)
                                            <p style="margin: 0; font-size: 14px; color: #666;"><strong>Notes:</strong> {{ $order->notes }}</p>
                                        @endif
                                    </td>
                                </tr>
                            </table>
                        ...
                        <td style="padding: 10px 35px 30px;">
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color: #f8f9fa; border-radius: 8px; padding: 20px;">
                                <tr>
                                    ...
                                    <td align="right" style="font-size: 14px; color: #333; padding: 4px 0;">${{ number_format($order->subtotal, 2) }}</td>
                                </tr>
                                @if($order->discount_amount > 0)
                                    <tr>
                                        <td style="font-size: 14px; color: #dc3545; padding: 4px 0;">Discount ({{ $order->coupon_code }}):</td>
                                        <td align="right" style="font-size: 14px; color: #dc3545; padding: 4px 0;">-${{ number_format($order->discount_amount, 2) }}</td>
                                    </tr>
                                @endif
                                <tr>
                                    <td style="font-size: 14px; color: #555; padding: 4px 0;">Shipping:</td>
                                    <td align="right" style="font-size: 14px; color: #333; padding: 4px 0;">${{ number_format($order->shipping, 2) }}</td>
                                </tr>
                                <tr>
                                    <td style="font-size: 14px; color: #555; padding: 4px 0;">Tax:</td>
                                    <td align="right" style="font-size: 14px; color: #333; padding: 4px 0;">${{ number_format($order->tax, 2) }}</td>
                                </tr>
                                <tr style="border-top: 1px solid #ddd;">
                                    <td style="font-size: 16px; font-weight: 700; color: #1a1a2e; padding: 10px 0 0;">Total Amount:</td>
                                    <td align="right" style="font-size: 16px; font-weight: 700; color: #0f3460; padding: 10px 0 0;">${{ number_format($order->total, 2) }}</td>
                                </tr>
                            </table>
                        </td>
                    </tr>

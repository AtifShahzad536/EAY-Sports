...
@section('header', 'Wholesale Order Breakdown')
...
@section('content')
    ...
    <div class="mb-4 d-flex justify-content-between align-items-center">
        <a href="{{ route('admin.dealer-orders.index') }}" class="btn btn-sm btn-outline-secondary">
            <i class="bi bi-chevron-left me-1"></i> Back to Orders Log
        </a>
        <a href="{{ route('admin.dealer-orders.pdf', $order->id) }}" target="_blank" class="btn btn-sm btn-danger d-flex align-items-center gap-1.5 fw-bold shadow-sm">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm rounded-3 bg-white p-4 mb-4">
                <h5 class="fw-bold border-bottom pb-3 mb-4 text-dark uppercase tracking-wide">Customized Products Invoice</h5>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        <thead class="bg-light text-muted small text-uppercase">
                            <tr>
                                ...
                                <th class="py-3">Custom Specifications</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex flex-column gap-1.5 align-items-start">
                                            ...
                                            @endif
                                            @if($item->savedDesign)
                                                <span class="border fw-bold text-center" style="color: #2563eb !important; background-color: #eff6ff !important; border-color: #bfdbfe !important; font-size: 11px; padding: 6px 12px; display: inline-block; border-radius: 50px; min-width: 90px; text-shadow: none;">3D Design: {{ $item->savedDesign->name }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="fw-semibold text-dark">{{ $item->qty }} units</td>
                                    <td>${{ number_format($item->unit_price, 2) }}</td>
                                    <td class="text-end pe-3 fw-bold text-dark">${{ number_format($item->unit_price * $item->qty, 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <!-- Notes / Wholesale Requests -->
                @if($order->notes)
                    <div class="border-top pt-4 mt-4">
                        <small class="text-muted d-block text-uppercase fw-semibold mb-1.5" style="font-size: 11px; letter-spacing: 0.5px;">B2B Special Custom Requests</small>
                        <p class="text-secondary fw-semibold mb-0 bg-light p-3 rounded border italic" style="line-height: 1.6; font-size: 13px;">
                            "{{ $order->notes }}"
                        </p>
                    </div>
                @endif
                                                            ...
                                                            try {
                                                                ...
                                                                targetMeshes.forEach(function(targetMesh) {
                                                                    try {
                                                                        var geo;
                                                                        if (d.type === 'pattern') {
                                                                            // For patterns: clone mesh geometry directly instead of DecalGeometry.
        ...
        <div class="col-lg-4">
            <!-- Status Update -->
            <div class="card border-0 shadow-sm rounded-3 bg-white p-4 mb-4">
                <h5 class="fw-bold border-bottom pb-3 mb-4 text-dark uppercase tracking-wide">Production Status & Notes</h5>
                
                <form action="{{ route('admin.dealer-orders.updateStatus', $order->id) }}" method="POST" class="space-y-4">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label text-xs font-bold text-muted uppercase">Set Delivery Stage</label>
                    ...
                    <button type="submit" class="btn btn-primary w-100 py-2.5 fw-bold d-flex align-items-center justify-content-center gap-1 shadow-sm">
            ...
            <div class="card border-0 shadow-sm rounded-3 bg-white p-4 mb-4">
                ...
                <div class="space-y-3 small fw-semibold text-slate-500">
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <strong class="text-dark">{{ $order->dealer ? $order->dealer->name : 'Unknown' }}</strong>
                    </div>
                    <div class="d-flex justify-content-between mb-2">
                        <span>Total Units:</span>
                        <strong class="text-dark">{{ $order->items->sum('qty') }} jerseys/shorts</strong>
                    </div>
                    <div class="d-flex justify-content-between mb-3 border-bottom pb-2">
                        <span>Shipping Address:</span>
                        <div class="text-end">
                            <strong class="text-dark d-block">{{ $order->shipping_address }}</strong>
                    ...
                    <div class="d-flex justify-content-between align-items-center pt-2">
                        ...
                        <h4 class="mb-0 fw-extrabold text-primary">${{ number_format($order->total_price, 2) }}</h4>
                    </div>
                </div>
            </div>
        </div>
